...
public class SwipeCloseActionBarWebActivity extends BaseActionBarWebActivity {
    ...
    private void createMaskView() {
        if (maskView != null)
            return;
        maskView = new View(SwipeCloseActionBarWebActivity.this);
    ...
    public void loadUrl(PageInfo info) {
        if (getPages().size() > 0) {//switch page in activity, screenshot for swipe close url
            //screenshot except webView
            getTitleBar().setDrawingCacheEnabled(true);
            exceptWebView = Bitmap.createBitmap(getTitleBar().getDrawingCache());
            getTitleBar().setDrawingCacheEnabled(false);

            //screenshot webView
            webView.captureBitmap(new WebViewEngine.GetBitmapCallback() {
                @Override
                public void onGetBitmap(@NotNull Bitmap b) {
                    pushBg(exceptWebView, b);
                }
            });
    ...
    private void pushBg(Bitmap exceptWebView, Bitmap webView) {
        int top = getTitleBar().getVisibility() == View.VISIBLE ? getTitleBar().getHeight() : 0;
